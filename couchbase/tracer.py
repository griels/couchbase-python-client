...
class ProxySpan(Span):
    ...
    def __init__(
        ...
        super(ProxySpan, self).__init__(tracer, context)
        self._tracer = tracer
        self._lock = Lock()
    ...
    def set_operation_name(self, operation_name):
        with self._lock:
            self.operation_name = operation_name
        return super(ProxySpan, self).set_operation_name(operation_name)
    ...
    def set_tag(self, key, value):
        with self._lock:
            ...
            if self.tags is None:
                self.tags = {}
            self.tags[key] = value
        return super(ProxySpan, self).set_tag(key, value)

    def log_kv(self, key_values, timestamp=None):
        with self._lock:
            self.logs.append(LogData(key_values, timestamp))
        return super(ProxySpan, self).log_kv(key_values, timestamp)
