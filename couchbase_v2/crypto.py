...
from couchbase_v2 import CryptoProvider
import ctypes.util
import ctypes
import logging
import os.path
    ...
    def __init__(self, providername, providerfunc, initfuncs=[], providerpaths=[], keystore=None, **kwargs):
        ...
        for library in providerpaths:
            ...
            try:
                ...
                logging.error(str(e))
                raise couchbase_v2.exceptions.NotFoundError("Couldn't load provider shared library")
        if crypto_dll:
            ...
            try:
                ...
                raise couchbase_v2.exceptions.NotFoundError("Couldn't initialise crypto provider shared library")

            super(CTypesCryptoProvider, self).__init__(provider=provider)
            if self.keystore:
                setattr(self, 'load_key', self.key_loader)
        else:
            raise couchbase_v2.exceptions.NotFoundError("Couldn't find provider shared library")
