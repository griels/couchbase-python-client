...
static void pycbc_Context_enumerate_chiidren(pycbc_stack_context_handle context, int should_clean_subnodes) {
    pycbc_context_children* current_node = context->children;
    PYCBC_DEBUG_LOG("freeing children of %p, should already all be free!", context);
    while (current_node)
    {
        pycbc_context_children* next_node = current_node->next;
        PYCBC_DEBUG_LOG("context %p: freeing node %p", context, current_node->value);
...
static void pycbc_Context_ref(pycbc_stack_context_handle ref_context, pycbc_stack_context_handle child) {
    pycbc_context_children* child_node = calloc(1, sizeof(pycbc_context_children));
...
pycbc_stack_context_handle
pycbc_Result_start_context(pycbc_stack_context *parent_context, PyObject *hkey, char *component,
                           char *operation) {
    ...
    if (PYCBC_CHECK_CONTEXT(parent_context)) {
        ...
        if (py_tracer) {
            stack_context_handle = pycbc_Context_init(py_tracer, operation, 0, parent_context, LCBTRACE_REF_CHILD_OF, component);
        }
...
pycbc_stack_context_handle pycbc_MultiResult_extract_context(pycbc_MultiResult *self, PyObject *hkey, pycbc_Result** res) {
    ...
    if( *res ) {
        PYCBC_DEBUG_PYFORMAT("[%R]\n"
                             "&res %p:  coming back from callback on key: [%R]", mrdict, res, hkey);
...
void pycbc_MultiResult_init_context(pycbc_MultiResult *self, PyObject *curkey,
    ...
    PYCBC_DEBUG_PYFORMAT("res %p: binding context %p to [%R]", item, context, curkey);
    PYCBC_EXCEPTION_LOG_NOCLEAR;
...
typedef struct pycbc_tracer_span_args {
    PYCBC_X_SPAN_ARGS(PYCBC_TAG_TEXT,PYCBC_TAG_ULL,PYCBC_TAG_STRUCT,PYCBC_TAG_ULL)
...
#define PYCBC_CCBC_TO_OT_ID(NAME)// if (args->NAME) {  pycbc_set_dict_kv_object(dict, pycbc_##NAME, ); }

PyObject* pycbc_set_args_from_payload(pycbc_tracer_span_args_t *args) {
    PyObject* dict = PyDict_New();

    PYCBC_X_SPAN_ARGS(PYCBC_TEXT_TO_DICT,PYCBC_ULL_TO_DICT,TAGS,PYCBC_CCBC_TO_OT_ID);
    return dict;
}
...
void pycbc_span_args_dealloc(pycbc_tracer_span_args_t *args) {
    PYCBC_X_SPAN_ARGS(PYCBC_TEXT_FREE,PYCBC_ULL_FREE, PYCBC_TAGS_FREE, PYCBC_ULL_FREE)
    free(args);
}
...
void pycbc_Tracer_enqueue_payload(pycbc_tracer_state *state, pycbc_tracer_payload *payload) {
    if (state->last) {
...
void pycbc_Tracer_span_finish(const struct pycbc_tracer_payload *payload, const pycbc_tracer_state *state,
                              PyObject *fresh_span);
...
pycbc_tracer_payload* pycbc_Tracer_propagate_span(pycbc_Tracer_t *tracer, struct pycbc_tracer_payload *payload) {
    ...
    if (state->start_span_method && PyObject_IsTrue(state->start_span_method)) {
        ...
        {
            PyObject *key = PyLong_FromUnsignedLongLong(*payload->span_start_args->child_of);
        ...
        PYCBC_DEBUG_PYFORMAT("calling start method: %R ( %R )", state->start_span_method,start_span_args);

        {
            PyObject *fresh_span = PyObject_Call(state->start_span_method, pycbc_DummyTuple,
                                                              start_span_args);
            if (fresh_span) {
...
void pycbc_Tracer_span_finish(const struct pycbc_tracer_payload *payload, const pycbc_tracer_state *state,
    ...
    if (finish_method) {
        ...
        PYCBC_DEBUG_PYFORMAT("calling finish method with;[%R]",span_finish_args);
...
void pycbc_tracer_flush(pycbc_Tracer_t *tracer)
{
    ...
    }
    {
        ...
        while (ptr) {
            ...
            ptr = ptr->next;
            if (state->parent) {
                pycbc_tracer_payload* requeue = pycbc_Tracer_propagate_span(tracer, tmp);
                if (requeue)
                {
                    pycbc_tracer_payload* old_root = state->root;
                    state->root = requeue;
                    requeue->next = old_root;
                    continue;
                }
            }
...
lcbtrace_TRACER *pycbc_tracer_new(PyObject *parent, lcbtrace_TRACER *child_tracer)
{
    ...
    if (parent) {
        PYCBC_DEBUG_PYFORMAT("initialising tracer start_span method from:[%R]",parent);
        pycbc_tracer->start_span_method = PyObject_GetAttrString(parent, "start_span");
        if (!PyErr_Occurred() && pycbc_tracer->start_span_method)
        {
            PYCBC_DEBUG_PYFORMAT("got start_span method:[%R]", pycbc_tracer->start_span_method);
...
void pycbc_Tracer_set_child(pycbc_Tracer_t *py_Tracer, lcbtrace_TRACER *child_tracer) {
...
Tracer__init__(pycbc_Tracer_t *self,
    ...
    lcbtrace_TRACER *child_tracer = (lcbtrace_TRACER *) (threshold_tracer_capsule ? PyCapsule_GetPointer(
            threshold_tracer_capsule,
            "threshold_tracer") : NULL);
    self->tracer = parent ? pycbc_tracer_new(parent,
                                             child_tracer) : child_tracer;
