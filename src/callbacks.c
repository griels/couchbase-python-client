...
static void getcid_callback(lcb_t instance,
                            int cbtype,
                            const lcb_RESPBASE *resp_base)
{
    ...
    if (context->result.err != LCB_SUCCESS) {
        fprintf(stderr,
                "%-20.*s, %-20.*s Failed to get collection ID: %s\n",
        ...
        lcb_respgetcid_collection_id(resp,
                                     &context->result.value.collection_id);
        lcb_respgetcid_manifest_id(resp, &context->result.value.manifest_id);
        printf("%-20.*s, %-20.*s ManifestId=0x%02" PRIx64
               ", CollectionId=0x%02x\n",
               (int)collection.length,
               (char *)collection.buffer,
               (int)scope.length,
