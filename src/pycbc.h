...
#    define PYCBC_FREE(X) \
        pycbc_free_debug(__FILE__, __FUNCTION_NAME__, __LINE__, X)
...
#    define PYCBC_CMD_SET_KEY(TYPE, CMD, BUF, LEN)                           \
        PYCBC_DEBUG_LOG(                                                     \
                "Setting key %.*s on cmd %s at %llx", LEN, BUF, #TYPE, CMD); \
...
#    define CMDSCOPE(UC, LC, ...) \
        CMDSCOPE_GENERIC(UC, LC, PYCBC_DUMMY, __VA_ARGS__)
#    define CMDSCOPE_FAIL(UC)    \
        {                        \
            cmd_done = 1;        \
            goto GT_##UC##_DONE; \
...
#    define lcb_cmdgetreplica_create_all(ptr) \
        (*(ptr))->strategy = LCB_REPLICA_ALL
...
//#define lcb_cmdhttp_host(CMD, HOST, NHOST) (CMD)->host=HOST;
//PYCBC_DUMMY((CMD)->nhost=NHOST);
...
#    define lcb_cmdview_no_row_parse(VCMD, ENABLE)                    \
        (VCMD)->cmdflags =                                            \
                ((VCMD)->cmdflags & ~LCB_CMDVIEWQUERY_F_NOROWPARSE) | \
...
#    define lcb_respgetcid_collection_id(RESP, DEST) \
        *(DEST) = (RESP)->collection_id
...
pycbc_coll_res_t *result); pycbc_coll_res_t
pycbc_Collection_get_cid(pycbc_Collection *collection);
*/
#ifdef PYCBC_NATIVE_COLLECTIONS
pycbc_Unit pycbc_Bucket_init_collection(pycbc_Collection *bucket,
                                        PyObject *args,
                                        PyObject *kwargs)
{
    return bucket;
...
#        define GENERIC_NULL_OPERAND(                           \
                SCOPE, COMMAND, INSTANCE, HANDLE, CONTEXT, ...) \
...
#        else
#            define GENERIC_SPAN_OPERAND(                  \
                    SCOPE, INSTANCE, CMD, HANDLE, CONTEXT) \
...
PyMODINIT_FUNC init_libcouchbase(void);
#endif
