                                   ...
                                   pycbc_stack_context_handle context)
{
    ...
    pycbc_Collection *coll = pycbc_Bucket_init_collection(
            self, pycbc_DummyTuple, pycbc_DummyKeywords);
    return pycbc_oputil_iter_multi_Collection(
        ...
        pycbc_stack_context_handle context)
{
    ...
    for (ii = 0; ii < orig_ncmds; ii++) {
        ...
        rv = PYCBC_TRACE_WRAP_EXPLICIT_NAMED(&context,
                                             ...
                                             itm,
                                             arg);
        /*        rv = pycbc_wrap_and_pop_debug("_file_name_", 462,
           "_function_name_", (handler).name, &context, 1,
                                              ((handler).cb(NULL, self, cv,
           optype, arg_k, v, options, itm, arg,
                                                            pycbc_explicit_named_setup("_file_name_",
           462, "_function_name_", &context, (handler).name, (handler).category,
           ((void *) 0), self->bucket))), cv);
                           ...
                           size_t index)
{
    ...
    pycbc_sdspec_details details = {.op = op,
                                    .flags = flags,
                                    .pathbuf = pathbuf,
                                    .valbuf = valbuf,
                                    .index = index};
