...
#ifdef PYCBC_TRACING
    return self->context_capsule ? PyCapsule_GetPointer(self->context_capsule, "tracing_context") : NULL;
#else
    return NULL;
#endif
}
...
TRACED_FUNCTION_WRAPPER(_view_request, LCBTRACE_OP_REQUEST_ENCODING, Bucket)
{
    ...
    vres = pycbc_propagate_view_result(context);//PYCBC_TYPE_CTOR(&pycbc_ViewResultType);
...
ViewResult_fetch(pycbc_ViewResult *self, PyObject *args)
{
    ...
    if (!self->base.done) {
        pycbc_oputil_wait_common(bucket, pycbc_viewresult_get_context(self));//PYCBC_TRACE_GET_STACK_CONTEXT_TOPLEVEL(NULL, LCBTRACE_OP_RESPONSE_DECODING, self->py_tracer, "viewresult.fetch"));
    }
                   ...
                   PyObject *args, PyObject *kwargs)
{
    ...
    PyObject* context_capsule=kwargs?PyDict_GetItemString(kwargs, "context"):NULL;
